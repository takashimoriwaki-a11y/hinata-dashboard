...
export function AttendanceCheckModal({ type, onClose, onConfirm, checkoutChecklistUrl, isEmergency }: AttendanceCheckModalProps) {
  ...
  const [overtimeContactTargets, setOvertimeContactTargets] = useState<Record<string, string>>(savedState?.overtimeContactTargets ?? {});
  const [overtimeRecordCounts, setOvertimeRecordCounts] = useState<Record<string, number>>(savedState?.overtimeRecordCounts ?? {});
  ...
  const buildOvertimeReason = (): string => {
    ...
    const parts = overtimeReasonTypes.map((reason) => {
      switch (reason) {
        ...
        case "支援者連絡": {
          const v = overtimeContactTargets["支援者連絡"];
          return v ? `支援者連絡（${v}）` : "支援者連絡";
        }
        case "家族連絡": {
          const v = overtimeContactTargets["家族連絡"];
          return v ? `家族連絡（${v}）` : "家族連絡";
        }
        case "記録書Ⅱ作成": return `記録書Ⅱ作成（${overtimeRecordCounts["記録書Ⅱ作成"] ?? 1}人分）`;
        case "月次報告書作成": return `月次報告書作成（${overtimeRecordCounts["月次報告書作成"] ?? 1}人分）`;
        case "状態報告書作成": return `状態報告書作成（${overtimeRecordCounts["状態報告書作成"] ?? 1}人分）`;
  ...
  const handleOvertimeSubmit = () => {
    ...
    const contactReasons = (["支援者連絡", "家族連絡"] as const).filter((r) => overtimeReasonTypes.includes(r));
    const missingContact = contactReasons.find((r) => !(overtimeContactTargets[r] ?? "").trim());
    if (missingContact) {
      toast.error(`${missingContact}の連絡先を入力してください`);
      return;
    }
  ...
  const handleAlcoholOnly = () => {
    ...
    const contactReasonsAlc = (["支援者連絡", "家族連絡"] as const).filter((r) => overtimeReasonTypes.includes(r));
    const missingContactAlc = contactReasonsAlc.find((r) => !(overtimeContactTargets[r] ?? "").trim());
    if (!isClockIn && hasOvertime && missingContactAlc) {
      toast.error(`${missingContactAlc}の連絡先を入力してください`);
      return;
    }
    ...
    saveAlcoholCheckMutation.mutate({
      ...
      overtimeContact: ((): string | undefined => {
        ...
        const parts = (["支援者連絡", "家族連絡"] as const)
          .filter((r) => overtimeReasonTypes.includes(r))
          .map((r) => ({ r, v: (overtimeContactTargets[r] ?? "").trim() }))
      ...
      overtimeCount: ((): number | undefined => {
        if (!(!isClockIn && hasOvertime)) return undefined;
        const counts = (["記録書Ⅱ作成", "月次報告書作成", "状態報告書作成"] as const)
          .filter((r) => overtimeReasonTypes.includes(r))
          .map((r) => overtimeRecordCounts[r] ?? 1);
    ...
    <div className={`mx-3 mt-2 mb-0 rounded-xl border-2 ${
      ...
      <div className="px-4 py-4 space-y-4 bg-white dark:bg-gray-900">
          ...
          <div className="flex gap-2">
            {(["online", "face"] as const).map((method) => (
              <button
                ...
                className={`flex-1 py-2.5 text-sm font-medium rounded-xl border-2 transition-all active:scale-95 cursor-pointer ${
                  confirmMethod === method
                    ? isClockIn
                      ? "bg-orange-500 border-orange-500 text-white shadow-sm"
                      : "bg-cyan-500 border-cyan-500 text-white shadow-sm"
                    : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500"
                }`}
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            {([
              ...
              <button
                ...
                className={`py-2 text-sm font-medium rounded-xl border-2 transition-all active:scale-95 cursor-pointer ${
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-1.5 py-2.5 mt-1 rounded-xl text-xs font-semibold transition-all active:scale-95 cursor-pointer ${
            isClockIn
    ...
    <div ref={overtimeCardRef} className="mx-3 my-2 rounded-xl border-2 border-blue-200 dark:border-blue-800" style={{overflow: 'clip'}}>
      <div className={`flex items-center ${hasOvertime ? "bg-blue-50 dark:bg-blue-950/30" : "bg-gray-50 dark:bg-gray-800"}`}>
        <button
          ...
          className={`flex-1 flex items-center justify-between px-4 py-3 text-sm font-semibold transition-all active:scale-[0.98] cursor-pointer ${
            hasOvertime
        ...
        <div className="px-4 pb-4 pt-3 space-y-3 bg-white dark:bg-gray-900">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button type="button" onClick={() => { setOvertimeReasonTypes([]); setOvertimeContactTargets({}); setOvertimeRecordCounts({}); setOvertimeFreeText(""); }} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 px-1.5 py-0.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">リセット</button>
            </div>
              ...
              {["訪問看護実施", "支援者連絡", "家族連絡", "記録書Ⅱ作成", "月次報告書作成", "状態報告書作成", "その他"].map((reason) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 px-2 text-xs font-medium rounded-xl border-2 transition-all active:scale-95 cursor-pointer ${
                      isSelected
                        ? "bg-blue-500 border-blue-500 text-white shadow-sm"
                        : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-950/20 hover:border-blue-400 dark:hover:border-blue-600"
                    }`}
                  >
          ...
          {!isOfficeStaff && (["支援者連絡", "家族連絡"] as const).filter((r) => overtimeReasonTypes.includes(r)).map((reason) => (
            <div key={reason}>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                ...
                {reason}：誰に連絡したか <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={overtimeContactTargets[reason] ?? ""}
                onChange={(e) => setOvertimeContactTargets((prev) => ({ ...prev, [reason]: e.target.value }))}
                placeholder={reason === "支援者連絡" ? "例: 相談支援専門員 山田さん" : "例: ○○様の長女 鈴木さん"}
                className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-0 focus:ring-blue-400"
                style={{ fontSize: "16px" }}
              />
            </div>
          ...
          {!isOfficeStaff && (["記録書Ⅱ作成", "月次報告書作成", "状態報告書作成"] as const).filter((r) => overtimeReasonTypes.includes(r)).map((reason) => (
            <div key={reason}>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                ...
                {reason}：何人分 <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <select
                  value={overtimeRecordCounts[reason] ?? 1}
                  onChange={(e) => setOvertimeRecordCounts((prev) => ({ ...prev, [reason]: Number(e.target.value) }))}
          ...
          <div className="px-4 pb-3 pt-0">
            <button
              ...
              onClick={() => void handleStepButton(step)}
                className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg transition-all active:scale-95 ${
                isOpening ? "opacity-60 cursor-wait" : "cursor-pointer"
              } ${
                isClockIn
  ...
  return (
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            onClick={() => {
              ...
              setOvertimeContactTargets({});
              setOvertimeRecordCounts({});
              setOvertimeFreeText("");
          ...
          <AlertDialogDescription asChild>
            <div className="space-y-3 text-sm">
              ...
              <div className="bg-blue-50 dark:bg-blue-950/30 rounded-xl p-3 space-y-2">
                ...
                {/* 連絡先（支援者連絡・家族連絡それぞれ） */}
                {(["支援者連絡", "家族連絡"] as const).filter((r) => overtimeReasonTypes.includes(r) && (overtimeContactTargets[r] ?? "").trim()).map((reason) => (
                  <div key={reason} className="flex items-start justify-between gap-2">
                    ...
                    <span className="font-medium text-foreground text-right">{overtimeContactTargets[reason]}</span>
                  </div>
                ))}
                {/* 人数（記録書系それぞれ） */}
                {(["記録書Ⅱ作成", "月次報告書作成", "状態報告書作成"] as const).filter((r) => overtimeReasonTypes.includes(r)).map((reason) => (
                  <div key={reason} className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-foreground">{overtimeRecordCounts[reason] ?? 1}人分</span>
                  </div>
                ))}
                {/* 自由記述（その他が含まれる場合） */}
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center touch-none" style={{height: '100svh', overflow: 'hidden', maxHeight: '100svh', overscrollBehavior: 'none'}}>
      ...
      {/* モーダル本体 */}
      <div
        className="relative w-full sm:max-w-md mx-0 sm:mx-4 bg-white dark:bg-gray-900 rounded-b-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[100svh] overflow-hidden"
      >
            ...
            <>
              ...
              <div className="mx-3 my-2">
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all active:scale-95 cursor-pointer ${
                    voiceMemoDeleted
              ...
              <div className="mx-3 my-2">
                {checkoutChecklistUrl ? (
                  <a
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all shadow-md active:scale-95 cursor-pointer bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/30 dark:hover:bg-blue-900/40 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-700"
                  >
        ...
        {/* 退勤時フッター（閉じるのみ） */}
        {!isClockIn && (
          <div className="px-5 pt-2 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 bg-white dark:bg-gray-900" style={{paddingBottom: 'max(1rem, env(safe-area-inset-bottom, 1rem))'}}>
            <button
              type="button"
              onClick={handleCloseRequest}
