...
【施設・組織名・地域名】
ひなた、こころの訪問看護ステーションひなた、光陽、株式会社光陽、あかり、生活介護事業所あかり、
ハートランドしぎ、医療法人ハートランドしぎ、
大和郡山市、天理市、奈良県、安堵町、斑鳩町、川西町、三郷町、王寺町、上牧町、広陵町、
郡山北部、郡山南部、
ZEST、iBow、
大和郡山病院、奈良県立医科大学附属病院、天理市立病院、南奈良総合医療センター、
奈良県西和医療センター、吉田病院、三笠病院、やまと精神医療センター、
大和郡山市保健センター、天理市保健センター、
地域活動支援センター、就労継続支援、就労移行支援、放課後等デイサービス、
ショートステイ、短期入所、グループホーム、障害者支援施設、共同生活援助、

【訪問看護算定・制度関連】
精神科訪問看護基本療養費、精神科複数回訪問加算、精神科緊急訪問看護加算、
長時間精神科訪問看護加算、精神科訪問看護指示書、特別訪問看護指示書、
医療保険、介護保険、障害福祉サービス、自立支援医療（精神通院）、
訪問看護ステーション、管理者、統括所長、

【よく使う略語・口語表現】
ケアマネ、ケアマネジャー、ヘルパー、OT（作業療法士）、PT（理学療法士）、ST（言語聴覚士）、
PSW（精神保健福祉士）、SW（ソーシャルワーカー）、Dr（ドクター・医師）、
Ns（ナース・看護師）、CW（ケースワーカー）、
バイタル、BP（血圧）、HR（心拍数）、RR（呼吸数）、BT（体温）、
Sp（SpO2・酸素飽和度）、GCS、JCS、
ADL、IADL、QOL、
`;
...
【病状の経過記録に特化した追加指示】
以下の表現を正確に認識してください：
- 症状の変化：「改善」「悪化」「増悪」「軽快」「安定」「不安定」「波がある」「変わらない」
- 精神状態：「落ち着いている」「不穏」「興奮状態」「混乱」「錯乱」「意識清明」「緊張感がある」「フラット」
- 服薬状況：「服薬できている」「服薬拒否」「飲み忘れ」「自己中断」「過量服薬」「追加服薬」「剤形」「注射」
- 生活状況：「自室にこもっている」「外出できている」「食事摂取できている」「睡眠がとれている」「清潔できている」
- 家族状況：「家族の負担が大きい」「介護疲れ」「家族支援が必要」「家族の理解が得られている」
- 訪問時の様子：「笑顔が見られた」「アイコンタクトあり」「会話のキャッチボールができた」「話しかけに応じる」
- 身体状況：「浮腫（むくみ）」「血圧高め」「血圧低め」「顔色不良」「呼吸苦しい」「座位保持」「食欲低下」
- 全体的な印象：「活気がある」「元気そう」「表情が明るい」「小話ができた」「主動的に話してくれた」
`,
      task: `${BASE_TERMS}
...
`,
      handoff_memo: `${BASE_TERMS}

【訪問後の申し送り内容に特化した追加指示】
スタッフが訪問後に、次の担当者やチームへ共有する内容を話しています。以下を特に正確に認識してください：
- 申し送り表現：「申し送り」「引き継ぎ」「共有」「確認してほしい」「次回確認」「要フォロー」「要注意」「継続確認」
- 利用者の状態：「表情」「発語」「睡眠」「食事」「服薬」「拒薬」「飲み忘れ」「頓服」「不穏」「落ち着いている」「不安」「幻聴」「妄想」「希死念慮」
- 家族・関係機関：「家族」「母」「父」「娘」「息子」「ケアマネ」「相談支援専門員」「主治医」「ヘルパー」「グループホーム」
- 訪問看護の確認事項：「バイタル」「BP」「SpO2」「体温」「浮腫」「疼痛」「転倒」「清潔」「入浴」「排泄」「水分摂取」
- 書類・制度：「受給者証」「自立支援」「保険証」「同意書」「計画書」「報告書」「指示書」
- 日時・頻度：「次回」「明日」「来週」「午前」「午後」「何時」「何日」「週1回」「週2回」

文字起こし方針：
1. 音声の内容を忠実に文字起こしする。ここでは要約や整文はしない。
2. 訂正表現（「違う」「じゃなくて」「やっぱり」「ちゃう」など）も聞こえた通りに残す。
3. 利用者名・スタッフ名・施設名は登録名候補を参考にし、最も近い固有名詞を正確に表記する。
4. 医療・看護・福祉用語を一般語に崩さない。
5. 数値、日時、回数、単位を落とさない。
`,
    };
...
// ========== 25-31日（JST）に翌月分アルコールチェックスプレッドシートを自動作成・DB登録（堅牢版） ==========
// 旧仕様: 25日9:00ちょうどの1分間のみ → サーバー再起動で取りこぼしリスクあり
// 新仕様: 25-31日の毎時0分 + サーバー起動時にも実行 → 取りこぼし防止
function scheduleNextMonthAlcoholSheet() {
  const checkInterval = 60 * 1000; // 1分ごとにチェック
  let lastCreatedMonth = "";

  // 翌月分のアルコールチェックシートを作成する処理（cron + 起動時の両方から呼ぶ）
  async function tryCreateNextMonthAlcoholSheet() {
    const now = new Date();
    const jstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
    ...
    // 同じ月で一度処理済みならスキップ
    if (lastCreatedMonth === monthKey) return;
    ...
    try {
      ...
      if (existing) {
        ...
        lastCreatedMonth = monthKey; // その月の処理は完了とみなす
        return;
      }
  ...
  // 25-31日の毎時0分にチェック（24回 × 7日 = 168回のチャンス）
  setInterval(async () => {
    const now = new Date();
    const jstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
    const m = jstNow.getUTCMinutes();
    ...
    // 25-31日の毎時0分に実行
    if (d >= 25 && d <= 31 && m === 0) {
      await tryCreateNextMonthAlcoholSheet();
    }
  }, checkInterval);

  // サーバー起動時にも一度実行（25-31日なら）
  (async () => {
    const now = new Date();
    const jstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
    const d = jstNow.getUTCDate();
    if (d >= 25 && d <= 31) {
      console.log(`[AlcoholSheetAuto] サーバー起動時チェック（${d}日）`);
      await tryCreateNextMonthAlcoholSheet();
    }
  })();

  console.log("[AlcoholSheetAuto] 25-31日の毎時0分・サーバー起動時に翌月スプレッドシート自動作成スケジューラーを開始しました");
}
scheduleNextMonthAlcoholSheet();

// ========== 25-31日（JST）に翌月分出退勤スプレッドシートを自動作成・DB登録（堅牢版） ==========
// 旧仕様: 25日9:05ちょうどの1分間のみ → サーバー再起動で取りこぼしリスクあり
// 新仕様: 25-31日の毎時5分 + サーバー起動時にも実行 → 取りこぼし防止
function scheduleNextMonthTimesheetSheet() {
  const checkInterval = 60 * 1000; // 1分ごとにチェック
  let lastCreatedMonth = "";

  // 翌月分の出退勤シートを作成する処理（cron + 起動時の両方から呼ぶ）
  async function tryCreateNextMonthTimesheet() {
    const now = new Date();
    const jstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
    const currentYear = jstNow.getUTCFullYear();
    const currentMonth = jstNow.getUTCMonth() + 1;
    ...
    try {
      ...
      console.log(`[TimesheetSheetAuto] ${nextMonthLabel}分のスプレッドシートを自動作成します`);
  ...
  // 25-31日の毎時5分にチェック（アルコールシートと5分ずらす）
  setInterval(async () => {
    const now = new Date();
    const jstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
    const m = jstNow.getUTCMinutes();
    ...
    // 25-31日の毎時5分に実行
    if (d >= 25 && d <= 31 && m === 5) {
      await tryCreateNextMonthTimesheet();
    }
  }, checkInterval);
  ...
  (async () => {
    ...
    const d = jstNow.getUTCDate();
    if (d >= 25 && d <= 31) {
      console.log(`[TimesheetSheetAuto] サーバー起動時チェック（${d}日）`);
      await tryCreateNextMonthTimesheet();
    }
  })();

  console.log("[TimesheetSheetAuto] 25-31日の毎時5分・サーバー起動時に翌月出退勤スプレッドシート自動作成スケジューラーを開始しました");
}
